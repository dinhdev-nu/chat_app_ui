...
export default function NotificationCenter({ notifications, onClose, onMarkAllAsRead }: NotificationCenterProps) {
    ...
    <Card className="w-80 bg-[#1e1f2e]/95 backdrop-blur-md border border-indigo-500/20 shadow-lg overflow-hidden">
      <div className="p-3 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center">
          ...
          {unreadCount > 0 && (
            <span className="ml-2 bg-indigo-600 text-white text-xs rounded-full px-2 py-0.5">{unreadCount}</span>
          )}
        </div>
            ...
            {notifications.map((notification) => (
              <motion.div
                ...
                className={`p-3 rounded-lg mb-2 ${
                  notification.read ? "bg-[#1a1b26]/50" : "bg-indigo-500/10 border border-indigo-500/20"
                }`}
              >
                <div className="flex">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-300 mt-1">{notification.content}</div>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      {unreadCount > 0 && (
        <div className="p-3 border-t border-white/10">
          <Button onClick={onMarkAllAsRead} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white">
